...
const roleLabelKey = (r: Role) => `users.role${r.charAt(0).toUpperCase()}${r.slice(1)}`;
const statusLabelKey = (s: UserStatus) => `users.status${s.charAt(0).toUpperCase()}${s.slice(1)}`;
  ...
  return (
    <Dialog open={!!user} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Edit className="w-5 h-5 text-primary" /> {t("users.editTitle")}</DialogTitle>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="space-y-2">
            ...
            <Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder={t("common.optionalCap")} dir="ltr" />
          </div>
              ...
              <Select value={role} onValueChange={(v) => setRole(v as Role)}>
                ...
                <SelectContent>
                  {(["admin","directeur","professeur","parent"] as Role[]).map((r) => (
                    <SelectItem key={r} value={r}>{t(roleLabelKey(r))}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
              ...
              <Select value={status} onValueChange={(v) => setStatus(v as UserStatus)}>
                ...
                <SelectContent>
                  {(["active","pending","suspended"] as UserStatus[]).map((s) => (
                    <SelectItem key={s} value={s}>{t(statusLabelKey(s))}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="flex gap-2 justify-end pt-1">
            <Button variant="outline" onClick={onClose} disabled={saving}><X className="w-4 h-4 me-1.5" /> {t("common.cancel")}</Button>
...
function LinkChildrenDialog({ parent, onClose, fetchStudents, getParentChildren, setParentChildren }: LinkChildrenDialogProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t("users.childrenLinked", { count: selected.length, name: parent!.fullName }));
      onClose();
    } catch (e: unknown) {
      setErr(e instanceof Error ? e.message : t("common.error"));
    } finally {
  ...
  return (
    <Dialog open={!!parent} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><UserCheck className="w-5 h-5 text-primary" /> {t("users.linkTitle")}</DialogTitle>
          <DialogDescription>{t("users.linkDesc", { name: parent?.fullName ?? "" })}</DialogDescription>
        </DialogHeader>
        <div className="space-y-4 pt-2">
          {err && <p className="text-sm text-destructive">{err}</p>}
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-4">{t("users.noStudents")}</p>
          ) : (
          ...
          {selected.length > 0 && (
            <p className="text-xs text-primary font-medium">{t("users.studentsSelected", { count: selected.length })}</p>
          )}
          <div className="flex gap-2 justify-end pt-1">
            <Button variant="outline" onClick={onClose} disabled={saving}><X className="w-4 h-4 me-1.5" /> {t("common.cancel")}</Button>
      ...
      <CardContent className="p-4 flex items-center justify-between gap-4 flex-wrap">
        <div className="space-y-0.5">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge className={roleBadgeClass(user.role)}>{t(roleLabelKey(user.role))}</Badge>
            <Badge variant={statusBadgeVariant(user.status)} className="text-xs">{t(statusLabelKey(user.status))}</Badge>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {onLinkChildren && user.role === "parent" && (
            <Button size="sm" variant="outline" onClick={() => onLinkChildren(user)} className="gap-1.5">
              <UserCheck className="w-3.5 h-3.5" /> {t("users.linkChildren")}
            </Button>
          )}
...
export default function UserManagementPage() {
  ...
  const filterByTab = (list: AdminUser[], activeTab: string) => {
    if (activeTab === "all") return list;
    return list.filter((u) => u.role === activeTab);
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <ShieldCheck className="w-6 h-6 text-primary" /> {t("users.title")}
          </h1>
        ...
        <Button variant="outline" size="sm" onClick={() => fetchUsers()} disabled={loading}>
          <RefreshCw className={`w-4 h-4 me-2 ${loading ? "animate-spin" : ""}`} /> {t("common.refresh")}
        </Button>
      </div>

      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="mb-4">
          <TabsTrigger value="all">{t("users.tabAll", { count: users.length })}</TabsTrigger>
          <TabsTrigger value="admin">{t("users.tabAdmins", { count: users.filter((u) => u.role === "admin").length })}</TabsTrigger>
          <TabsTrigger value="directeur">{t("users.tabDirecteurs", { count: users.filter((u) => u.role === "directeur").length })}</TabsTrigger>
          <TabsTrigger value="professeur">{t("users.tabTeachers", { count: users.filter((u) => u.role === "professeur").length })}</TabsTrigger>
          <TabsTrigger value="parent">{t("users.tabParents", { count: users.filter((u) => u.role === "parent").length })}</TabsTrigger>
        </TabsList>

        {["all","admin","directeur","professeur","parent"].map((tabValue) => (
          <TabsContent key={tabValue} value={tabValue}>
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 gap-2 text-muted-foreground">
                ...
                <p className="text-sm">{t("users.emptyCategory")}</p>
              </div>
            ) : (
              <div className="space-y-2">
                {displayed.map((u) => (
